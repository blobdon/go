...
func prove(f *Func) {
	...
	// should be sufficient to learn that len(w) <= cap(w) at the
	// beginning of prove where we look for all len/cap ops.
	if ft.caps != nil && ft.lens != nil {
		for sID, c := range ft.caps {
			// Len should always exist, but check to be sure
			if l, has := ft.lens[sID]; has {
				// TODO(zdjones): Should the block be from l, or c?
				addRestrictions(c.Block, ft, signed|unsigned, l, c, lt|eq)
			}
